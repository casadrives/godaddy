...
export function CommissionReports({ dateRange }: CommissionReportsProps) {
  ...
  const generateAllReports = async () => {
    ...
    try {
      for (const report of reports) {
        ...
        // Add report details
        doc.setFontSize(12);
        doc.text(`Driver: ${report.driver}`, 20, 55);
        doc.text(`Generated: ${new Date().toLocaleString()}`, 20, 65);
        
        // Add payment deadline warning
        doc.setTextColor(255, 0, 0);
        doc.text('IMPORTANT: Payment must be made within 24 hours to avoid account suspension', 20, 80);
        doc.setTextColor(0, 0, 0);
        
        // Add commission summary
        doc.text('Commission Summary', 20, 100);
        const data = [
        ...
        // Save the PDF
        doc.save(`commission-report-${report.driver.toLowerCase().replace(' ', '-')}-${dateRange.startDate}-to-${dateRange.endDate}.pdf`);
